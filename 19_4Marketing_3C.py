...
cv.fit(train.iloc[:, 1:], train['target_flag'])
#%%
...
test_p = cv.predict_proba(test.iloc[:, 1:])
...
list(zip(train.iloc[:, 1:].columns, imp))
...
train_p = cv.predict_proba(train.iloc[:, 1:])
test_p = cv.predict_proba(test.iloc[:, 1:])
print(roc_auc_score(test.target_flag, test_p[:, 1]))
...
gbc.fit(train.iloc[:, 1:], train.target_flag)
gbc_train_p = gbc.predict_proba(train.iloc[:, 1:])
gbc_test_p = gbc.predict_proba(test.iloc[:, 1:])
#%%
